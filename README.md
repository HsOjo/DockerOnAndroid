...
`configure` tunes the install per device: `crun-nomq` (no IPC_NS), `utsns = "host"` (no UTS_NS), a cgroup-v1 mount service (controllers not mounted at boot) and the storage driver (kernel overlayfs, fuse-overlayfs, or vfs) are each enabled only when needed; the podman wrapper is always installed — its ENOTEMPTY auto-recovery for stale post-reboot mount metadata ([podman#23504](https://github.com/containers/podman/issues/23504)) applies to every device, not just userns-less kernels. Re-running `configure` + `install.sh` reconciles the system — wrappers are restored from `*.real` when no longer needed. (A netavark native-bridge route was evaluated and dropped: Android kernels commonly ship read-only filter tables or trimmed xt matches, which no userspace workaround can fix.)
...
- **The gateway address answers nothing**: pasta runs with `--no-map-gw` (mapping the gateway to host loopback would steal the DNS traffic aardvark-dns receives on its gateway lo-alias), so the gateway IP is only a route target — it answers neither ICMP echo nor TCP/UDP. To reach the host, use its real interface address.
...
- **EXPOSE connectivity relies on an asynchronous inspect** (querying the container's own config right after start); under extreme timing the first rule may arrive seconds late. Under `podman run` the image link is invisible until start-commit, so applying the EXPOSE rules restarts pasta once (~1s window) — an entrypoint that pings without retry can lose its first packet. Standalone `podman create` + `podman start` carries the rules in the first pass and has no window.
...
- **Lost `merged` mounts make container removal stickily fail (auto-recovered)**: if a container starts while its `merged` dir is not actually mounted (stale "mounted" storage metadata after a crash or unclean reboot — upstream [podman#23504](https://github.com/containers/podman/issues/23504), unfixed; seen on fuse-overlayfs *and* native overlay — any `restart: always` container racing boot can hit it), podman writes its per-container config (`etc/hosts`, `resolv.conf`, `hostname`, `run/.containerenv`, plus empty skeleton dirs) into the *plain* dir, and every later `rm` / `compose down` dies at `replacing mount point ".../merged": directory not empty` — retrying is futile and the storage container leaks into `podman ps --storage`. The podman wrapper's `rm`/`stop` auto-cleans when the dir holds *only* those known podman-generated files (podman-compose is covered too — it shells out to the podman CLI); any other file means real container data written while the overlay was lost, which is preserved and reported with manual recovery steps. When this fires, capture `ps aux | grep fuse-overlayfs`, the merged dir listing and `/proc/mounts` first — the open question is what killed the FUSE daemon (suspect: Android LMK under load).
